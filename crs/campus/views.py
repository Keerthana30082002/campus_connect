...
from .models import stu_details  # Import your model
from django.contrib.auth.models import Group
...
def student_register(request):
    if request.method == "POST":
        form = Student_SignUpForm(request.POST)
        if form.is_valid():
            # Save the form data to a new instance of stu_details
            student = stu_details(
                username=form.cleaned_data.get("username"),
                email=form.cleaned_data.get("email"),
                # Add other fields here as needed
            )
            student.save()  # Save to the stu_details model

            # If you want to log the user in after registration
            raw_password = form.cleaned_data.get("password1")
            user = authenticate(username=student.username, password=raw_password)
            if user is not None:
                login(request, user)
...
def ccd(request):
    if (
        ...
        if request.method == "POST":
            ...
            form = ccdForm()
            return render(
                request, "campus/ccd.html", {"form": form, "x": x, "y": y, "z": z}
            )
    else:
        return HttpResponse("<h1>u r not logged in</h1>")
...
def jobpos(request):
    if (
        ...
        if request.method == "POST":
            ...
            return render(
                request, "campus/jobpos.html", {"form": form, "x": x, "y": y1}
            )
    else:
        return HttpResponse("<h1>u r not logged in</h1>")
        ...
        if request.method == "POST":
            ...
            if b == "it":
                y = job_pos.objects.filter(
                    salary__gte=sal, bond_years__lte=bon, information_technology="yes"
                ).order_by("salary")
            if b == "cse":
                y = job_pos.objects.filter(
                    salary__gte=sal, bond_years__lte=bon, cse="yes"
                ).order_by("salary")
            if b == "me":
                y = job_pos.objects.filter(
                    salary__gte=sal, bond_years__lte=bon, mech="yes"
                ).order_by("salary")
            if b == "ce":
                y = job_pos.objects.filter(
                    salary__gte=sal, bond_years__lte=bon, civil="yes"
                ).order_by("salary")
            if b == "eee":
                y = job_pos.objects.filter(
                    salary__gte=sal, bond_years__lte=bon, eee="yes"
                ).order_by("salary")
            if b == "ece":
                y = job_pos.objects.filter(
                    salary__gte=sal, bond_years__lte=bon, ece="yes"
                ).order_by("salary")
            if b == "ch":
                y = job_pos.objects.filter(
                    salary__gte=sal, bond_years__lte=bon, chemical="yes"
                ).order_by("salary")
            print(y)
            if len(y) == 0:
...
def selectstu(request):
    ...
    if (
        ...
        if request.method == "POST":
            ...
            x = applied_jobs.objects.filter(job_id=jobid, company_id=u).values(
                "student_id"
            )
            y = []
            print(x)
    ...
    if (
        ...
        if request.method == "POST":
            ...
            body = (
                "Congratlations!!!"
                + str(name)
                + " you are selected for the interview ,the date for the interview will be anounced by your Placement Officer"
            )
